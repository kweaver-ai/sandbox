...
from sandbox_runtime.python_runner.base_runner import BaseCodeRunner
from python_runner.exec_runner import ExecRunner
...
class JupyterGatewayRunner(BaseCodeRunner):
    ...
    def __init__(
        self, kernel_id: Optional[str] = "", gateway_url: Optional[str] = None
    ):
        """
    ...
    def get_working_context(
        self, output_limit: int = -1, output_lines_limit: int = -1
    ) -> str:
        ...
        def compress_output_lines(output_lines: List[str]) -> List[str]:
            if output_lines_limit == -1:
                ...
                return (
                    output_lines[: output_lines_limit // 2]
                    + ["..."]
                    + output_lines[-output_lines_limit // 2 :]
                )
        ...
        for i, entry in enumerate(self.code_history):
            ...
            if status == "success":
                ...
                output_text = outcome.get(
                    "output", "(No output before error)"
                )  # Output might contain the traceback
                output_text = compress_output_text(output_text)
                output_lines.append(f"  Error Type: {error_type}")
                output_lines.append(f"  Error Message: {error_msg}")
                output_lines.append(
                    f"  Output (including error details):\n{output_text}"
                )
            else:
    ...
    def _get_or_create_kernel(self, kernel_id: Optional[str] = ""):
        ...
        if not JupyterGatewayRunner.is_jupyter_gateway_available():
            DEFAULT_LOGGER.warning("依赖未安装，无法连接到Jupyter Gateway")
            return None, None

        # 如果未提供gateway_url，则无法连接
        if not self.gateway_url:
            DEFAULT_LOGGER.warning(
                "未提供Jupyter Gateway URL，无法连接到Jupyter Gateway"
            )
            return None, None
        ...
        def _check_kernel_id(kernel_id: str):
            ...
            response = requests.get(kernel_url, verify=False, timeout=10)
        ...
        try:
            ...
            if kernel_id == "" or not _check_kernel_id(kernel_id):
                if kernel_id == "":
                    ...
                    DEFAULT_LOGGER.debug(f"创建kernel: {kernel_url}")

                    kernel_data = {"name": "python3"}

                    response = requests.post(
                        kernel_url, json=kernel_data, verify=False, timeout=10
                    )

                    if response.status_code != 201 and response.status_code != 200:
                        DEFAULT_LOGGER.error(
                            f"创建Jupyter Gateway kernel失败，状态码: {response.status_code}, 内容: {response.text}"
                        )
                        return None, None
                    ...
                    try:
                        ...
                        DEFAULT_LOGGER.error(
                            f"解析kernel信息失败: {str(e)}, 响应内容: {response.text}"
                        )
                        return None, None
                    ...
                    DEFAULT_LOGGER.debug(
                        f"创建了新的Jupyter Gateway kernel，ID: {kernel_id}"
                    )
                else:
                    ...
                    DEFAULT_LOGGER.debug(
                        f"连接到现有的Jupyter Gateway kernel，ID: {kernel_id}"
                    )
    ...
    def _connect_websocket(self):
        ...
        try:
            if not self._id:
                DEFAULT_LOGGER.error("未找到有效的kernel ID，无法创建WebSocket连接")
                return False
            ...
            DEFAULT_LOGGER.debug(f"连接WebSocket: {ws_url}")

            self.ws = websocket.create_connection(
                ws_url, sslopt={"cert_reqs": 0}, timeout=30
            )

            return True
        except Exception as e:
            DEFAULT_LOGGER.error(f"连接WebSocket失败: {str(e)}")
            return False
    ...
    def _close_websocket(self):
        ...
        if self.ws:
            try:
                self.ws.close()
                self.ws = None
                DEFAULT_LOGGER.debug("已关闭WebSocket连接")
            except Exception as e:
                DEFAULT_LOGGER.error(f"关闭WebSocket连接失败: {str(e)}")

    def run(
        self, code: str, data: Optional[Dict] = None, **namespace_kwargs
    ) -> Dict[str, Any]:
        """
        ...
        try:
            while retry_count <= max_retries:
                ...
                try:
                    ...
                    if not is_retry and (
                        not self.session
                        or not JupyterGatewayRunner.is_jupyter_gateway_available()
                    ):
                        DEFAULT_LOGGER.warning(
                            "Jupyter Gateway未安装或连接不可用，回退到ExecRunner"
                        )
                        try:
                            exec_result = ExecRunner.static_run(
                                code, data, **namespace_kwargs
                            )
                            outcome = {
                                "status": "success",
                                "result": exec_result,
                            ...
                            self.code_history.append(
                                {"code": original_code, "outcome": outcome}
                            )
                            ...
                            raise SandboxError(
                                message="ExecRunner代码执行失败", detail=exec_e
                            )  # 重新抛出包装后的错误

                    # --- WebSocket 连接与执行 ---
                    if not self.ws:
                        DEFAULT_LOGGER.info(
                            f"尝试连接 WebSocket (尝试次数 {retry_count + 1})"
                        )
                    ...
                    for key, value in namespace_kwargs.items():
                        try:
                            ...
                            DEFAULT_LOGGER.warning(f"无法序列化变量 {key}，跳过")
                    full_code += code
                    ...
                    self.ws.send(json.dumps(message))
                    DEFAULT_LOGGER.debug(
                        f"已发送代码执行请求，消息ID: {msg_id} (尝试次数 {retry_count + 1})"
                    )
                    ...
                    while time.time() < timeout:
                        try:
                            ...
                            if msg_type == "execute_result":
                                ...
                                if parent.get("msg_id") == msg_id:
                                    ...
                                    try:
                                        ...
                                        result = (
                                            text_result  # Keep original if eval fails
                                        )
                                ...
                                formatted_tb = "\n".join(
                                    [ansi_escape.sub("", line) for line in tb]
                                )
                                error_output = (
                            ...
                            DEFAULT_LOGGER.warning("WebSocket接收超时")
                            ...
                            DEFAULT_LOGGER.error(
                                f"WebSocket连接在接收时关闭: {conn_closed_err}"
                            )
                            self.ws = None
                            raise conn_closed_err  # Propagate to retry logic
                        except json.JSONDecodeError as json_err:
                            DEFAULT_LOGGER.error(f"解析WebSocket消息失败: {json_err}")
                            execution_error = json_err
                            break
                        except Exception as e:
                            DEFAULT_LOGGER.error(
                                f"处理WebSocket消息时发生意外错误: {str(e)}"
                            )
                            execution_error = e
                            break
                    ...
                    DEFAULT_LOGGER.warning(
                        f"捕获到连接关闭异常 (尝试次数 {retry_count + 1})，准备重试..."
                    )
                    ...
                    if retry_count <= max_retries:
                        ...
                        DEFAULT_LOGGER.error(
                            f"连接关闭，达到最大重试次数 ({max_retries})，放弃执行"
                        )
                        outcome_status = "error"
                        last_exception = SandboxError(
                            message="Jupyter Gateway连接失败 (已重试)",
                            detail=last_exception,
                        )
                    ...
                    DEFAULT_LOGGER.error(
                        f"执行过程中发生未处理的异常 (尝试次数 {retry_count + 1}): {e}"
                    )
                    last_exception = e
                    outcome_status = "error"
                    break  # Exit retry loop, will go to finally block
            ...
            DEFAULT_LOGGER.debug(
                f"Entering finally block. Detected outcome_status: {outcome_status}"
            )
            ...
            if outcome_status == "success":
                ...
                DEFAULT_LOGGER.debug(f"Recording successful outcome: {final_outcome}")
                self.code_history.append(
                    {"code": original_code, "outcome": final_outcome}
                )
                ...
                if isinstance(last_exception, SandboxError) and isinstance(
                    last_exception.detail, str
                ):
                    error_detail = (
                        last_exception.detail
                    )  # Use detail from PythonCodeError if available

                # Avoid duplicating output if it's already in the error detail
                if (
                    full_output_str
                    and error_detail
                    and full_output_str not in error_detail
                ):
                    ...
                    if len(full_output_str.strip()) > len(str(last_exception)) + 20:
                        ...
                        full_output_str.strip() != str(last_exception).strip()
                    ):  # Add if different from error
                        error_detail += f"\n(Output: {full_output_str.strip()})"
                ...
                DEFAULT_LOGGER.debug(f"Recording error outcome: {final_outcome}")
                self.code_history.append(
                    {"code": original_code, "outcome": final_outcome}
                )

                # Wrap the exception if it's not already a PythonCodeError
                if isinstance(last_exception, SandboxError):
                    DEFAULT_LOGGER.error(
                        f"Raising existing PythonCodeError: {last_exception.reason}"
                    )
                    raise last_exception
                else:
                    wrapped_exception = SandboxError(
                        reason=f"Jupyter Gateway代码执行失败 ({type(last_exception).__name__})",
                        detail=error_detail,
                    )
                    DEFAULT_LOGGER.error(
                        f"Raising wrapped exception: {wrapped_exception.reason}"
                    )
        ...
        if self._id in JupyterGatewayRunner._kernels:
            try:
                ...
                if response.status_code >= 400:
                    DEFAULT_LOGGER.warning(
                        f"关闭kernel时遇到HTTP错误: {response.status_code}, {response.text}"
                    )

                del JupyterGatewayRunner._kernels[self._id]
                DEFAULT_LOGGER.debug(f"关闭了Jupyter Gateway kernel，ID: {self._id}")
                self.session = None
                ...
                DEFAULT_LOGGER.error(f"关闭Jupyter Gateway kernel失败: {str(e)}")
                return False
        return False
    ...
    def static_run(
        ...
        **namespace_kwargs,
        ...
        # 如果需要历史记录，必须使用实例方法
        DEFAULT_LOGGER.warning("使用 static_run 时，无法记录代码执行历史上下文")
        runner = JupyterGatewayRunner(kernel_id=kernel_id, gateway_url=gateway_url)
    ...
    def close_kernel(kernel_id: str) -> bool:
        ...
        if kernel_id in JupyterGatewayRunner._kernels:
            try:
                ...
                DEFAULT_LOGGER.debug(f"关闭了Jupyter Gateway kernel，ID: {kernel_id}")
                return True
            except Exception as e:
                DEFAULT_LOGGER.error(f"关闭Jupyter Gateway kernel失败: {str(e)}")
                return False
    ...
    def close_all_kernels() -> int:
        ...
        for kernel_id in all_kernel_ids:
            if JupyterGatewayRunner.close_kernel(kernel_id):
                count += 1
        DEFAULT_LOGGER.debug(f"关闭了所有Jupyter Gateway kernel，共{count}个")
        return count
...
def get_runner(
    gateway_url: str = "", kernel_id: Optional[str] = ""
) -> JupyterGatewayRunner:
    """获取Jupyter Gateway执行器"""
    global _JUPYTER_GATEWAY_RUNNER
    if _JUPYTER_GATEWAY_RUNNER is None:
        _JUPYTER_GATEWAY_RUNNER = JupyterGatewayRunner(
            gateway_url=gateway_url, kernel_id=kernel_id
        )
    DEFAULT_LOGGER.warning(
        "get_runner 返回的是单例 JupyterGatewayRunner，可能共享 kernel 和历史"
    )
